...
//判断职工是否存在，如果存在返回职工所在数组中的下标，如果不存在则返回-1
...
void WorkerManager::Mod_Emp() {
	...
	if (this->m_FileIsEmpty) {
		...
		if (index != -1) {
			...
			switch (DepId) {
			case 1:
				worker = new Employee(newId, newName, DepId);
				break;
			case 2:
				worker = new Manager(newId, newName, DepId);
				break;
			case 3:
				worker = new Boss(newId, newName, DepId);
				break;
			default:
				break;
			}
