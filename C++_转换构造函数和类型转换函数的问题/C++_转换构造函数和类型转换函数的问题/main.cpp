...
//注意事项
//1、一定要注意，有转换构造函数的情况下，不要在有参构造函数内，使用默认参数，否则会识别不出转换构造函数
//2、在使用运算符重载时，对参数使用引用方式时，要考虑是否能传过来，例如，临时对象是没有别名一说的，所以没有引用，根本就传不过来
//3、只使用转换构造函数和类型转换函数，而没有用+号重载。就只会出现先转一种，再转一种的情况，具体看=号左边的类型
